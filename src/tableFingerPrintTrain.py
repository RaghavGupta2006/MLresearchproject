...
"""
Training pipeline for Tabular Descriptors + Molecular Fingerprint Data
...
def smiles_to_pubchem_fingerprint(smiles: str, n_bits: int = 881) -> np.ndarray:
    """
     ...
     :
        np.ndarray: 881 
    """
    max_retries = 3  # Note: processed parameter
    base_delay = 1.0  # Note: processed parameter
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"PubChem HTTP  (  {attempt + 1}/{max_retries}): {str(e)}")
            if "503" in str(e):  # Note: processed parameter
                # Note: processed parameter
                time.sleep(base_delay * (2 ** attempt))
            continue
        except Exception as e:
            print(f" SMILES {smiles}  (  {attempt + 1}/{max_retries}): {str(e)}")
            time.sleep(base_delay * (2 ** attempt))
            continue

    print(f"SMILES {smiles}  ， 0 ")
    return np.zeros(n_bits, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    print(f" : {args.fingerprint},  : {args.feat_d}")
