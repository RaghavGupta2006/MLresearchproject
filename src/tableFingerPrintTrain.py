...
# 添加命令行参数选择指纹类型
parser.add_argument('--fingerprint', type=str, choices=['maccs', 'morgan', 'rdkit', 'atompair', 'pubchem'],
                    default='pubchem', help='Type of molecular fingerprint to use')
...
parser.add_argument('--out_f', type=str, help='Output file path',
                    default='../checkpoint/best_GrowTableFingerPrint.pth')
...
# 将SMILES转换为指纹（处理无效分子）
def smiles_to_fingerprint(smiles, fp_type='maccs', n_bits=2048):
    """
    将SMILES转换为指定的分子指纹
    支持的指纹类型: 'maccs', 'morgan', 'rdkit', 'atompair'
    """
...
import time  # 确保已经导入time模块
import random


# 修改后的smiles_to_pubchem_fingerprint函数
def smiles_to_pubchem_fingerprint(smiles: str, n_bits: int = 881) -> np.ndarray:
    """
    将SMILES转换为PubChem指纹（881位）

    参数:
        smiles: SMILES字符串
        n_bits: 指纹长度(PubChem固定881位)

    返回:
        np.ndarray: 881位的指纹向量
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            return np.array([int(bit) for bit in binary_str[-n_bits:]], dtype=np.float32)

        except pcp.PubChemHTTPError as e:
            print(f"PubChem HTTP错误 (尝试 {attempt + 1}/{max_retries}): {str(e)}")
            if "503" in str(e):  # 服务不可用错误
                # 指数退避策略
                time.sleep(base_delay * (2 ** attempt))
            continue
        except Exception as e:
            print(f"处理SMILES {smiles}时出错 (尝试 {attempt + 1}/{max_retries}): {str(e)}")
            time.sleep(base_delay * (2 ** attempt))
            continue

    print(f"SMILES {smiles} 的所有尝试均失败，返回全0指纹")
    return np.zeros(n_bits, dtype=np.float32)
...
if __name__ == "__main__":
    ...
    # 确定指纹位数（MACCS固定167位，其他自定义）
    # 确定指纹位数
    if args.fingerprint == 'maccs':
        fp_bits = 167
    elif args.fingerprint == 'pubchem':
        fp_bits = 881  # PubChem指纹固定881位
    else:
        fp_bits = 2048  # 其他指纹默认2048位

    # 为所有SMILES生成指定类型的指纹特征
    if args.fingerprint == 'pubchem':
        print("正在生成PubChem指纹...")
        fingerprint_features = np.array([
            smiles_to_pubchem_fingerprint(smiles)
            for smiles in smiles_list
        ])
        print("生成成功")
    else:
        fingerprint_features = np.array([
            smiles_to_fingerprint(smiles, args.fingerprint, fp_bits)
            for smiles in smiles_list
        ])

    # 检查是否有无效SMILES
    invalid_mask = (fingerprint_features.sum(axis=1) == 0)
    if invalid_mask.any():
        print(f"警告：发现 {invalid_mask.sum()} 个无效SMILES，已自动填充全0指纹")

    # 合并数值特征和指纹特征
    X = np.hstack([X.astype(np.float32), fingerprint_features])

    # 更新特征维度参数
    args.feat_d = X.shape[1]
    print(f"使用指纹类型: {args.fingerprint}, 总特征维度: {args.feat_d}")
    ...
    # 将结果和超参数保存到日志文件
    log_path = '../checkpoint/tableFingerPrintTrain_log.txt'
