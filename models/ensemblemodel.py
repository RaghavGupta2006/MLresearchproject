...
class DynamicNetForMLPGNN(object):
    ...
    def forward(self, x, graph_data):
        ...
        with torch.no_grad():
            for m in self.models:
                if middle_feat_cum is None:
                    middle_feat_cum, prediction = m(x, graph_data, middle_feat_cum)
                else:
...
class DynamicNetForMLPImage(object):
    def __init__(self, c0, lr):
        self.models = []
        self.c0 = c0
        self.lr = lr
...
class DynamicNetForMLPGNNImage(object):
    def __init__(self, c0, lr):
        self.models = []
        self.c0 = c0
        self.lr = lr
    ...
    def forward(self, x, graph_data, image_data):
        if len(self.models) == 0:
            return None, self.c0
        middle_feat_cum = None
        prediction = None
        with torch.no_grad():
            for m in self.models:
                if middle_feat_cum is None:
                    middle_feat_cum, prediction = m(x, graph_data, image_data, middle_feat_cum)
                else:
...
class DynamicNetForMLPKeys(object):
    def __init__(self, c0, lr):
        self.models = []
        self.c0 = c0
        self.lr = lr
